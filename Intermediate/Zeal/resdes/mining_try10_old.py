...
###################################################################
#from __future__ import print_function to plot graph
import numpy as np
import matplotlib.pyplot as plt
...
aLow=450                #Happiest
bHigh,bLow=449,420      #Happier
cHigh,cLow=419,400      #Happy
dHigh,dLow=399,387      #OK
eHigh,eLow=386,0        #Not sad
N = 255.00
green=(0/N,100/N,0/N)
...
def assignColors(myStates_info,popdensity):    
    ...
    for shapedict in myStates_info:
        statename = shapedict['NAME'].lower()
...
if __name__=='__main__':
        ...
        for k in range(0,len(file_list_500)):
                ...
                plot_dict_positive_tweets[state_name] = str(positive_tweets)
                plot_dict_negative_tweets[state_name] = str(negative_tweets)
                plot_dict_happy_score[state_name] = str(happy_score) 
        ...
        for key in plot_dict_positive_tweets:
                plot_dict_percent_pos_tweets[key] = str(float(int(plot_dict_positive_tweets[key]))/450*100)             
