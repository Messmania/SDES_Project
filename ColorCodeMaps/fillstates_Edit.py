...
def plotOnMap(popdensity):
    ...
    m = Basemap(llcrnrlon=-119,llcrnrlat=20,urcrnrlon=-64,urcrnrlat=49,projection='laea',lat_1=33,lat_2=45,lon_0=-95,lat_0=50)
    #m.drawmapboundary(fill_color='aqua')
    m.bluemarble()
    #m.etopo()
    #m.fillcontinents(color='aqua')
    m.fillcontinents()
    ...
    for nshape,seg in enumerate(m.myStates):
        ...
        if statenames[nshape] not in ['District of Columbia','Puerto Rico']:
            ...
            ax.add_patch(poly)
            #Adding text
            #~ if statenames[nshape].lower()=='california':
                #~ print "======================Seg=============================\n\n",seg
            #centroid=findMidPoint(seg)
            if statenames[nshape] not in state_list:
                state_list.append(statenames[nshape]) 
                centroid=findMidPoint(seg)
                #ax.text(centroid[0],centroid[1],statenames[nshape],fontsize=10,horizontalalignment='center')
